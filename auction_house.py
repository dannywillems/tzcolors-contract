...
class BatchTransfer:
    ...
    def item(from_, txs):
        return sp.set_type_expr(
            ...
            BatchTransfer.get_transfer_type()
        )
...
class Auction():
    def get_type():
        return sp.TRecord(
            ...
            bidder=sp.TAddress
        ).layout(("token_address",("token_id",("token_amount",("end_timestamp",("seller",("bid_amount","bidder")))))))
...
DEFAULT_ADDRESS = sp.address("tz1XP87WZycSXEdobWLwSDQKCDtM8huTQyQn")
AUCTION_EXTENSION_THRESHOLD = sp.int(60*5) # 5 minutes
BID_STEP_THRESHOLD = sp.mutez(100000)

class BaDaaSBeerFA2(sp.Contract):
    ...
    def initial_auction(self, batch_initial_auction):
        ...
        auction_id_runner = sp.local(
            'auction_id_runner',
            batch_initial_auction.auction_id_start
        )
        sp.for token_id in batch_initial_auction.token_ids:
            sp.verify(
                (~self.data.total_supply.contains(token_id)),
                message = FA2ErrorMessage.NOT_OWNER
            )
            sp.verify(
                token_id <= MAXIMAL_TOKEN_ID,
                message = FA2ErrorMessage.NOT_OWNER
            )
            to_user = LedgerKey.make(
                sp.self_address,
                token_id
            )
            self.data.ledger[to_user] = sp.nat(1)
            self.data.total_supply[token_id] = sp.nat(1)
            operator_user = AllowanceKey.make(
                sp.self_address,
                self.data.initial_auction_house_address,
                token_id
            )
            self.data.allowances[operator_user] = True
            auction_house = sp.contract(
                AuctionCreateRequest.get_type(),
                self.data.initial_auction_house_address,
                entry_point = "create_auction"
            ).open_some()
    ...
    def update_operators(self, update_operator_requests):
        sp.set_type(
            update_operator_requests,
            UpdateOperatorsRequest.get_type()
        )
        ...
        sp.for transfer in batch_transfers:
           sp.for tx in transfer.txs:
                sp.if (tx.amount > sp.nat(0)):
                    ...
                    sp.verify(
                        ((sp.sender == transfer.from_) | self.data.allowances.get(operator_user, False)),
                        message=FA2ErrorMessage.NOT_OWNER
                    )
    ...
    def balance_of(self, balance_of_request):
        ...
        responses = sp.local(
            ...
            sp.set_type_expr(
                sp.list([]),
                BalanceOfRequest.get_response_type()
            )
        )
...
class AuctionHouse(sp.Contract):
    ...
    def create_auction(self, create_auction_request):
        ...
        token_contract = sp.contract(
            BatchTransfer.get_type(),
            create_auction_request.token_address,
            entry_point = "transfer"
        ).open_some()
        sp.verify(
            create_auction_request.token_amount > 0,
            message = AuctionErrorMessage.TOKEN_AMOUNT_TOO_LOW
        )
        sp.verify(
            create_auction_request.end_timestamp >= sp.now.add_hours(MINIMAL_AUCTION_DURATION),
            message=AuctionErrorMessage.END_DATE_TOO_SOON
        )
        sp.verify(
            create_auction_request.end_timestamp <= sp.now.add_hours(MAXIMAL_AUCTION_DURATION),
            message=AuctionErrorMessage.END_DATE_TOO_LATE
        )
        sp.verify(
            create_auction_request.bid_amount >= MINIMAL_BID,
            message=AuctionErrorMessage.BID_AMOUNT_TOO_LOW
        )
        sp.verify(
            ~self.data.auctions.contains(create_auction_request.auction_id),
            message=AuctionErrorMessage.ID_ALREADY_IN_USE
        )
        ...
        sp.transfer([
            BatchTransfer.item(
                ...
                [sp.record(
                    ...
                    amount=create_auction_request.token_amount
                )]
            )],
            sp.mutez(0),
            token_contract
        ...
        self.data.auctions[create_auction_request.auction_id] = sp.record(
            token_address=create_auction_request.token_address,
            token_id=create_auction_request.token_id,
            token_amount=create_auction_request.token_amount,
    ...
    def bid(self, auction_id):
        ...
        sp.verify(
            sp.sender != auction.seller,
            message=AuctionErrorMessage.SELLER_CANNOT_BID
        )
        sp.verify(
            sp.amount >= auction.bid_amount + BID_STEP_THRESHOLD,
            message=AuctionErrorMessage.BID_AMOUNT_TOO_LOW
        )
        sp.verify(
            sp.now < auction.end_timestamp,
...
def test():
    ...
    scenario.h2("BaDaaSBeerFA2")
    #sp.address("KT1CpeSQKdkhWi4pinYcseCFKmDhs5M74BkU"))
    fa2 = BaDaaSBeerFA2(auction_house.address)
    scenario += fa2

    scenario.h2("Initial Auction")

    scenario.p("Kick-off initial Auctions")
    scenario += fa2.initial_auction(
        auction_id_start=sp.nat(0),
        token_ids=[sp.nat(0), sp.nat(1), sp.nat(2)]
    ).run(sender=admin)
    ...
    scenario.p("Bob bids")
    scenario += auction_house.bid(0).run(
        sender=bob,
        amount=sp.mutez(2000000),
        now=sp.timestamp(0).add_minutes(1)
    )
    ...
    scenario += auction_house.bid(0).run(
        ...
        now=sp.timestamp(0).add_minutes(2)
    )
    ...
    scenario += auction_house.bid(0).run(
        ...
        now=sp.timestamp(0).add_minutes(3)
    )

    scenario.p("Bob withdraws")
    scenario += auction_house.withdraw(0).run(
        sender=bob,
        amount=sp.mutez(2000000),
        now=sp.timestamp(0).add_minutes(5).add_days(5)
    )
    ...
    scenario += fa2.update_operators(
        [sp.variant('add_operator', sp.record(
            owner=bob.address,
            operator=auction_house.address,
            token_id=sp.nat(0)
        ))]
    ).run(sender=bob)
    ...
    scenario += auction_house.create_auction(
        sp.record(
            ...
            token_id=sp.nat(0),
            token_amount=sp.nat(1),
            end_timestamp=sp.timestamp(60*60),
            bid_amount=sp.mutez(100000)
        )
    ).run(sender=bob, now=sp.timestamp(0))
    ...
    scenario += auction_house.bid(0).run(
        ...
        now=sp.timestamp(60*60-5)
    )

    scenario.p("Alice rebids")
    scenario += auction_house.bid(0).run(
        sender=alice,
        amount=sp.mutez(600000),
        now=sp.timestamp(60*60+5*60-6)
    )
    ...
    scenario += auction_house.withdraw(0).run(
        sender=alice,
        amount=sp.mutez(0),
        now=sp.timestamp(60*60+5*60-6+5*60+1)
    )
    ...
    ).run(sender=alice, now=sp.timestamp(0))

    scenario.p("Bob funds in parallel another auction")
    scenario += fa2.initial_auction(
        auction_id_start=sp.nat(10),
        token_ids=[sp.nat(4),sp.nat(5),sp.nat(6)]
    ).run(sender=admin)
    scenario.p("Bob bids")
    scenario += auction_house.bid(10).run(
        sender=bob,
    ...
    scenario += auction_house.withdraw(0).run(
        sender=alice,
        amount=sp.mutez(0),
        now=sp.timestamp(3+60*60)
    )

    scenario.p("Bob withdraws")
    scenario += auction_house.withdraw(10).run(
        sender=bob,
        now=sp.timestamp(1+5*24*60*60)
    )
    ...
    ).run(sender=bob, now=sp.timestamp(0), valid=False)
    ...
    scenario += fa2.transfer([
        BatchTransfer.item(
            bob.address,
            [sp.record(to_=bob.address, token_id=0, amount=2)]
        )
    ]).run(sender=bob, valid=False)
    ...
    scenario += fa2.transfer([
        BatchTransfer.item(
            alice.address,
            [sp.record(to_=dan.address, token_id=0, amount=1)]
        )
    ]).run(sender=bob, valid=False)
    ...
    scenario += fa2.initial_auction(
        auction_id_start=sp.nat(123),
        token_ids=[sp.nat(1689)]
    ).run(sender=admin)

    scenario.p("Try too large token id")
    scenario += fa2.initial_auction(
        auction_id_start=sp.nat(124),
        token_ids=[sp.nat(1690)]
    ).run(sender=admin, valid=False)